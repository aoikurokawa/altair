...
use solana_program::{
    ...
    sysvar::{
        rent::{Rent, ID as RENT_PROGRAM_ID},
        Sysvar,
    },
};
...
pub fn open_lootbox(
    ...
    msg!("Mintint 10 tokens to User associated token account");
    invoke_signed(
...
pub fn add_comment(
    ...
    msg!("Mintint 5 tokens to User associated token account");
    invoke_signed(
        &spl_token::instruction::mint_to(
            token_program.key,
            token_mint.key,
...
mod tests {
    ...
    async fn test_add_movie_review_instruction() {
        ...
        let title = "Captain America".to_string();
        let rating = 3;
        let review = "Linked the movie".to_string();
        ...
        let init_ata_ix = create_associated_token_account(
            &payer.pubkey(),
            &payer.pubkey(),
            &mint,
            &spl_token::id(),
